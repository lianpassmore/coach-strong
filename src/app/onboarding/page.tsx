...
export default function OnboardingPage() {
  ...
  // Added types to parameters to fix "Implicit Any" errors
  ...
  const nextStep = () => step < 6 ? setStep(step + 1) : handleComplete();
  ...
  return (
    <main className="min-h-screen bg-brand-sand pb-24 text-brand-dark overflow-x-hidden flex flex-col">
      ...
      <header className="bg-brand-dark text-white pt-12 pb-14 px-6 rounded-b-[2.5rem] shadow-xl relative">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-6">
             ...
             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-light">Calibration: Step {step}/6</p>
             <div className="w-9" />
          </div>
      ...
      <div className="max-w-md mx-auto px-5 -mt-6 space-y-6 relative z-10 flex-1 w-full">
        {/* Fixed min-h per tailwind suggestion */}
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl border border-brand-sand min-h-112 flex flex-col">
          ...
          <div className="mt-auto pt-8">
             <button
               onClick={nextStep}
               disabled={isSaving || (step === 6 && !hasConsented)}
             ...
             >
               {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : step === 6 ? "Initialize My Plan" : "Continue"}
...
// Fixed TypeScript Props for Sub-components
